...
1. **Clustering Ward D2** sobre atributos físicos de cuenca ponderados con metadatos cualitativos (régimen hidrológico y subregión climática), con restricción mínima WMO de ≥ 5 cuencas por región. Hasta donde los autores conocen, constituye una de las **primeras aplicaciones documentadas de ponderación de metadatos cualitativos en clustering hidrológico para los Andes colombianos** en la literatura indexada.
2. **Exponente de Hurst (R/S)** para detección de memoria de larga dependencia y selección automática entre **ARFIMA(0,d,0)** y AR(1) para la generación de series sintéticas. Hasta donde los autores conocen, representa una de las **primeras cuantificaciones sistemáticas del exponente de Hurst para cuencas del río Cauca** en la literatura indexada, demostrando que la memoria de larga dependencia es un rasgo universal de este sistema andino, no excepcional.
3. **Modelo log-log de caudal medio** con selección de predictores por BIC (`MASS::stepAIC`), diagnóstico de influencia (distancia de Cook) e intervalos de predicción con corrección de Jensen. Se implementa explícitamente la **corrección de sesgo de Jensen**, práctica no siempre documentada en estudios de regionalización hidrológica colombianos en la literatura indexada, mejorando la consistencia de las predicciones en escala dimensional.
