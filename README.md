...
**Flow Duration Curve Regionalization via the Index-Flow Method with Long-Memory**
...
**[EN]** RegFDC-Cauca v1.0.0 is an open-source R framework for estimating flow duration curves (FDCs) in ungauged catchments of the Cauca River hydrological system (Colombian Andes). It implements the **index-flow method** with Ward D2 clustering of catchment attributes, Hurst exponent detection for long-memory in streamflow, and a BIC-selected log-log mean discharge model. Leave-one-out cross-validation over 20 gauged catchments yields NSE = 0.97, KGE = 0.96, and PBIAS = 0.3 %.
...
RegFDC-Cauca/
│
├── RegFDC_Cauca_1.0.0.R              # Script principal (1 440 líneas, 10 secciones)
├── RegFDC_Cauca_1.0.0_datos.xlsx     # Archivo de entrada (5 hojas)
│
├── manuscript/
│   ├── RegFDC_Cauca_1.0.0_ES.tex    # Manuscrito en español (formato EngrXiv)
│   ├── RegFDC_Cauca_1.0.0_EN.tex    # Manuscript in English (EngrXiv format)
│   └── figures/                      # Figuras G1–G9 (PNG 150 dpi)
│       ├── G1_FDC_dimensionales.png
│       ├── G2_FDC_regionales.png
│       ├── G3_Error_LOO_frecuencia.png
│       ├── G4_QQ_regional.png
│       ├── G5_Dendrograma_Ward.png
│       ├── G6_Serie_C021.png
│       ├── G7_Diagnostico_mu.png
│       ├── G8_Escorrentia_especifica.png
│       └── G9_Hurst_cuencas.png
│
├── resultados/                        # Directorio de salida (generado al ejecutar)
│   ├── RegFDC_Cauca_1.0.0_resultados.xlsx   # 14 hojas de resultados
│   ├── series_sinteticas_todas.csv           # Series diarias sintéticas
│   ├── series_sinteticas/                    # CSV por cuenca estimada (C021–C028)
│   ├── graficos/                             # G1–G9 en alta resolución
│   ├── A_diagnostico_tecnico.txt             # Informe técnico detallado
│   └── B_resultados_y_aplicabilidad.txt      # Resultados y guía de uso
│
├── README.md
└── LICENSE
```
...
## Archivo de datos de entrada / Input data file
...
## Parámetros de configuración clave / Key configuration parameters
...
## Salidas del análisis / Analysis outputs
...
## Región de estudio y cuencas / Study region and catchments
...
## Rango de aplicabilidad / Applicability range
...
- Kling H. et al. (2012). Runoff conditions in the upper Danube basin. *Journal of Hydrology*, 440–441, 192–201. https://doi.org/10.1016/j.jhydrol.2012.03.011
- Poveda G. et al. (2011). Hydroclimatology of Colombia. *Rev. Acad. Colomb. Cienc.*, 35(135), 317–336.
